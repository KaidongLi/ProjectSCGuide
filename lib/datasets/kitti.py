    ...
    def _load_kitti_annotation(self, index):
        ...
        with open(filename) as f:
            ...
            for ix, obj in enumerate(objs):
                ...
                difficult = 2
                if (y2 - y1 + 1) >= 40 and occ == 0 and trunc >= 0 and trunc <= 0.15:
                    difficult = 0
                elif (y2 - y1 + 1) >= 25 and (occ == 0 or occ == 1) and trunc >= 0 and trunc <= 0.3:
                    difficult = 1


                difficulties[ix] = difficult

                cls = self._class_to_ind[info[0].strip()]
